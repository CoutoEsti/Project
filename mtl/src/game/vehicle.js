// Vehicle dynamics — Ruelle's model (hop/src/vehicle/physics.js), copied so
// the two games drive the same. Changes for MTL, and only these:
//   - it runs in the map frame (x east, n north): yaw is a compass heading and
//     increasing yaw turns right, so the steering input is no longer mirrored;
//   - collisions happen outside, against height-aware solids (map/collide.js);
//   - `input.airborne` takes the tyres off the road: off a ramp, nothing but
//     drag acts until the wheels touch down.
//
// Vehicle dynamics.
//
...
// World frame (x east, n north): forward = (sin yaw, cos yaw), right = (cos yaw, -sin yaw).
...
export class Vehicle {
  ...
  get vn() { return Math.cos(this.yaw) * this.u - Math.sin(this.yaw) * this.v; }

  /** Set the velocity from a world-frame vector (after a collision). */
  setWorldVelocity(vx, vn) {
   ...
   * @param {{throttle,brake,steer,handbrake,airborne}} input
   */
  step(dt, input) {
    const S = this.spec;
    ...
    let driveForce = input.airborne ? 0 : (engineTorque * totalRatio * S.driveline) / S.wheelRadius;
    ...
    // Airborne, the tyres carry (almost) nothing.
    const air = input.airborne ? 0.02 : 1;
    const muF = S.grip * air;
    const muR = S.grip * S.rearGripBias * (handbrake ? S.handbrakeGripLoss : 1) * air;
    ...
    const sy = Math.sin(this.yaw), cy = Math.cos(this.yaw);
    const vxWorld = sy * this.u + cy * this.v;
    const vnWorld = cy * this.u - sy * this.v;
    ...
    if (!Number.isFinite(this.x) || !Number.isFinite(this.n) || !Number.isFinite(this.yaw)) {
